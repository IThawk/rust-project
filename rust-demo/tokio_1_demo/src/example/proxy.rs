    ...
    #[cfg(not(target_os = "windows"))]
        {
            ...
            let done = a
                ...
                .for_each(move |client| {
                    let server = UnixStream::connect("/root.socket");
                    let amounts = server.and_then(move |server| {
                        ...
                        let client_reader = MyUdsTcpStream(Arc::new(Mutex::new(client)));
                        let client_writer = client_reader.clone();
                        let server_reader = MyUdsTcpStream(Arc::new(Mutex::new(server)));
...
#[derive(Clone)]
struct MyUdsTcpStream(Arc<Mutex<UnixStream>>);

impl Read for MyUdsTcpStream {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        self.0.lock().unwrap().read(buf)
    }
}

impl Write for MyUdsTcpStream {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        self.0.lock().unwrap().write(buf)
    }
...
impl AsyncRead for MyUdsTcpStream {}

impl AsyncWrite for MyUdsTcpStream {
    fn shutdown(&mut self) -> Poll<(), io::Error> {
        self.0.lock().unwrap().shutdown(Shutdown::Write)?;
        Ok(().into())
    }
